...
int main(int argc, char *argv[])
{
    ...
    const std::size_t Ny = myInts.size();

    const std::string myString("Hello Variable String from rank " +
                               std::to_string(rank));

    std::cout << "helloBPWriterJeff.cpp..." << rank << std::endl;
    ...
    try
    {
       ...
       // ??  io.SetEngine("BPFile");
        ...
        adios2::Variable<float> &bpFloats = bpIO.DefineVariable<float>(
            "jeffFloats", {size * Nx}, {rank * Nx}, {Nx}, adios2::ConstantDims);

        adios2::Variable<int> &bpInts = bpIO.DefineVariable<int>(
            "jeffInts", {size * Nx}, {rank * Nx}, {Nx}, adios2::ConstantDims);
        ...
        adios2::Engine &bpFileWriter =
            bpIO.Open("myVector_cppJeff.bp", adios2::Mode::Write);
